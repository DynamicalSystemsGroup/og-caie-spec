...
<style>
  ...
  #synthbar{margin-left:10px;padding-left:10px;border-left:1px solid #2a2f3a}
  #synthbar button{background:#2a2f3a;color:#e6e6e6;border:1px solid #3a4150;border-radius:6px;padding:2px 8px;cursor:pointer;font:inherit;font-size:12px}
  #synthbar button[aria-pressed="false"]{background:#5d403233;border-color:#c78a5b;color:#ffd9b3}
  .node.dim{opacity:.12}
...
<header>
  ...
  <span id="synthbar" title="the worked example is a synthetic case (sheet 10-43): include or exclude its record and agents from every view">
    <button id="synthetic" aria-pressed="true">synthetic data: shown</button>
  </span>
...
let showSynthetic = true;  // sheet 10-43: the page can include or exclude the synthetic record; the data only carries the tag
const viewPresent = ()=>VIEWS[cur].present.filter(id=>showSynthetic || !(byId.get(id)&&byId.get(id).synthetic));
const view = ()=>({...VIEWS[cur], present: viewPresent()});
...
function readHash(){
  ...
  if(p.get("synthetic")==="0") showSynthetic = false;
...
function writeHash(){
  ...
  if(!showSynthetic) p.set("synthetic","0");
  try{ history.replaceState(null,"","#"+p.toString()); }catch(e){}
}
function syncSynthbar(){ const b=document.getElementById("synthetic"); b.setAttribute("aria-pressed", String(showSynthetic)); b.textContent = showSynthetic ? "synthetic data: shown" : "synthetic data: hidden"; }
document.getElementById("synthetic").addEventListener("click", ()=>{ showSynthetic = !showSynthetic; syncSynthbar(); if(selectedId && byId.get(selectedId).synthetic && !showSynthetic){ clearDetail(); } paint(); writeHash(); });
function openHash(h){
