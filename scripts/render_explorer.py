...
<aside id="detail"><div class="empty">click a node to read it; drag to pin, double-click to release</div></aside>
...
svg.call(d3.zoom().scaleExtent([.15,5]).on("zoom",ev=>g.attr("transform",ev.transform)));
...
function showDetail(id){
  ...
  selectedId = id;
  if(!view().present.includes(id)){ cur = VIEWS.findIndex(v=>v.present.includes(id)); if(cur<0) cur = VIEWS.length-1; paint(); } else repaintSelection();
...
svg.on("click",()=>{ if(selectedId){selectedId=null;repaintSelection();detailEl.textContent="";
  detailEl.appendChild(el("div","empty","click a node to read it; drag to pin, double-click to release"));}});
...
function shortIri(v){ for(const [k,ns] of Object.entries(PREFIX_MAP)) if(v.startsWith(ns)) return k+":"+v.slice(ns.length); return v; }
const PREFIX_MAP = Object.fromEntries(PREFIXES.trim().split("\n").map(l=>{const m=l.match(/^PREFIX (\w*): <([^>]*)>/);return m?[m[1],m[2]]:null;}).filter(Boolean));

function layout(){
  const hh = document.querySelector("header").offsetHeight;
  detailEl.style.top = (hh+12)+"px";
  detailEl.style.bottom = (document.getElementById("sparql").offsetHeight+24)+"px";
  const cy = hh + (H-hh)*.5; sim.force("center").y(cy); sim.force("y").y(cy);
}
paint(); layout();
