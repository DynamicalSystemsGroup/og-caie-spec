...
COUNTEREXAMPLES: dict[str, dict] = {
    # ---- older faults, restated on the measles evaluation (sheet 10-19)
    "attestation-without-evidence": dict(
        shapes=["S6-Attestation", "S8-Recommendation"],
        fault="attestation-1 says failed but aggregates no determination, so nothing behind it rules on evidence (the S6 closure rule); the recommendation that rests on it can then reach no evidence through it (S8).",
        replace=[("    prov:used ev:determination-1a ;\n    earl:result [ a earl:TestResult ; earl:outcome earl:failed ;\n                  earl:info \"The response ignored",
                  "    earl:result [ a earl:TestResult ; earl:outcome earl:failed ;\n                  earl:info \"The response ignored")]),
    "probe-before-requirements": dict(
        ...
        replace=[("ev:derivation-1 ;\n    prov:generatedAtTime \"2026-08-03T10:00:00Z\"", "ev:derivation-1 ;\n    prov:generatedAtTime \"2026-08-01T10:00:00Z\"")]),
    "recommendation-untraced": dict(
        ...
        replace=[("    prov:wasDerivedFrom ev:attestation-1 ; prov:wasDerivedFrom ev:attestation-2 ;\n", "")]),
    "attestation-off-plan": dict(
        shapes=["S6-Attestation"],
        fault="attestation-1, about a1, also aggregates determination-1b, a ruling on evidence that bears on a2 (the S6 chain rule, R-12, R-18).",
        replace=[("    prov:used ev:determination-1a ;\n    earl:result [ a earl:TestResult ; earl:outcome earl:failed ;\n                  earl:info \"The response ignored",
                  "    prov:used ev:determination-1a ; prov:used ev:determination-1b ;\n    earl:result [ a earl:TestResult ; earl:outcome earl:failed ;\n                  earl:info \"The response ignored")]),
    "attestation-off-turn": dict(
        shapes=["S6-Attestation"],
        fault="attestation-2, about a2, aggregates determination-1a, the ruling made on the evidence about a1 at the same turn: sessions are stateful and evidence answers one criterion; the ruling on a1 says nothing about a2 (the S6 chain rule, R-13).",
        replace=[("    prov:used ev:determination-1b ; prov:used ev:determination-2b ;\n    earl:result [ a earl:TestResult ; earl:outcome earl:failed ;\n                  earl:info \"The response does not advise",
                  "    prov:used ev:determination-1a ; prov:used ev:determination-1b ; prov:used ev:determination-2b ;\n    earl:result [ a earl:TestResult ; earl:outcome earl:failed ;\n                  earl:info \"The response does not advise")]),
    ...
    "expert-administers-tests": dict(
        ...
        fault="session-1 was run by Annie, the domain expert, whose slot is the DSO, the appropriateness assessment and the attestation, not administering tests (R-10, R-23); her own determination on the evidence from that session then stands alone, which independence at the person level forbids (sheet 10-15).",
        replace=[("    prov:wasAssociatedWith ev:chatbot-v1 ;\n    prov:wasAssociatedWith ev:theo ;", "    prov:wasAssociatedWith ev:chatbot-v1 ;\n    prov:wasAssociatedWith ev:annie ;")]),
    ...
    "attestation-before-determination": dict(
        shapes=["S6-Attestation"],
        fault="determination-1a ended after attestation-1, which aggregates it: an attestation ends no earlier than its determinations (sheet 10-19).",
        replace=[("    prov:endedAtTime \"2026-08-11T08:50:00Z\"", "    prov:endedAtTime \"2026-08-11T09:10:00Z\"")]),
    "consistency-check-after-turn": dict(
        shapes=["S3-Probe"],
        fault="the probe's consistency check ended after the first turn that used the probe had started (sheet 10-19).",
        replace=[("    prov:endedAtTime \"2026-08-03T10:05:00Z\"", "    prov:endedAtTime \"2026-08-10T14:30:00Z\"")]),
    "delivery-before-approval": dict(
        shapes=["S8-Delivery"],
        fault="the delivery was made before the report approval it carries had ended (sheet 10-19).",
        replace=[("    epo:deliveredTo ev:county-public-health-office ;\n    prov:generatedAtTime \"2026-08-12T11:00:00Z\"", "    epo:deliveredTo ev:county-public-health-office ;\n    prov:generatedAtTime \"2026-08-12T09:40:00Z\"")]),
    "plan-approved-after-session": dict(
        shapes=["S3-PlanApproval"],
        fault="the plan approval ended after the session using the plan's probes had started (sheet 10-19).",
        replace=[("    earl:result [ a earl:TestResult ; earl:outcome earl:passed ; earl:info \"the transit probe is the question commuters actually ask\" ] ;\n    prov:endedAtTime \"2026-08-03T11:00:00Z\"",
                  "    earl:result [ a earl:TestResult ; earl:outcome earl:passed ; earl:info \"the transit probe is the question commuters actually ask\" ] ;\n    prov:endedAtTime \"2026-08-10T15:00:00Z\"")]),
    "report-coverage-padded": dict(
        shapes=["S7-Report"],
        fault="the report stores a coverage of one while a3 carries no attestation: the coverage is recomputed from the record (walkthrough B5, R-48).",
        replace=[("    epo:coverage 0.75 ;", "    epo:coverage 1.0 ;")]),
    "report-rates-padded": dict(
        shapes=["S7-Report"],
        fault="the report stores a pass rate of one while every attestation failed; the coverage is right, the rates are recomputed (SCI-08, sheet 10-19).",
        replace=[("    epo:passRate 0.0 ; epo:failRate 1.0 ; epo:cantTellRate 0.0 ;", "    epo:passRate 1.0 ; epo:failRate 0.0 ; epo:cantTellRate 0.0 ;")]),
    "executive-attests": dict(
        ...
        replace=[("    earl:assertedBy ev:annie ;\n    prov:wasAssociatedWith ev:annie ;\n    prov:used ev:determination-1a ;", "    earl:assertedBy ev:mala ;\n    prov:wasAssociatedWith ev:annie ;\n    prov:used ev:determination-1a ;")]),
    ...
    "cherry-picked-determination": dict(
        shapes=["S6-Attestation"],
        fault="attestation-2 uses determination-1b and drops determination-2b, Annie's own ruling on the same evidence that ended before it, without naming it as excluded with a reason (no cherry-picking, sheet 10-14).",
        replace=[("    prov:used ev:determination-1b ; prov:used ev:determination-2b ;", "    prov:used ev:determination-1b ;")]),
    ...
    "operator-determines-alone": dict(
        ...
        fault="both determinations on a2 are Theo's, the operator who ran the session the evidence came from; independence at the person level asks for a second determination by another person (sheet 10-15).",
        replace=[("    earl:mode earl:manual ; earl:assertedBy ev:annie ;\n    prov:wasAssociatedWith ev:annie ;\n    prov:used ev:evidence-1b ;", "    earl:mode earl:manual ; earl:assertedBy ev:theo ;\n    prov:wasAssociatedWith ev:theo ;\n    prov:used ev:evidence-1b ;")]),
    "deviation-unrecorded": dict(
        shapes=["S3-TestPlan"],
        fault="a3 is left out of the plan and no plan deviation says why (sheet 10-16).",
        drop="plan-deviation-1"),
    "deviation-by-expert": dict(
        shapes=["S3-PlanDeviation"],
        fault="the plan deviation is attributed to Annie, the domain expert; departing from the plan is the operator's act to record (sheet 10-16).",
        replace=[("    epo:reason \"the exposure-on-transit criterion was left for a second round\" ;\n    prov:wasAttributedTo ev:theo ;", "    epo:reason \"the exposure-on-transit criterion was left for a second round\" ;\n    prov:wasAttributedTo ev:annie ;")]),
    "session-on-another-item": dict(
        shapes=["S4-Session"],
        fault="the session ran against a later build of the chatbot, not the version the requirement set binds and the access grant names (sheet 10-17).",
        replace=[("    prov:wasAssociatedWith ev:chatbot-v1 ;\n    prov:wasAssociatedWith ev:theo ;", "    prov:wasAssociatedWith ev:chatbot-v2 ;\n    prov:wasAssociatedWith ev:theo ;")],
        append=CHATBOT_V2),
    "insufficient-yet-failed": dict(
        shapes=["S6-Attestation"],
        fault="attestation-1 says failed on evidence it calls insufficient: insufficient evidence implies cannot tell (sheet 10-12, Z's rule).",
        replace=[("one such response shows the criterion unmet in that instance.\" ] ;\n    epo:appropriateness epo:appropriate ;\n    epo:sufficiency epo:sufficient ;",
                  "one such response shows the criterion unmet in that instance.\" ] ;\n    epo:appropriateness epo:appropriate ;\n    epo:sufficiency epo:insufficient ;")]),
    ...
    "recommendation-unapproved": dict(
        ...
        replace=[("    prov:wasDerivedFrom ev:report-approval-1 ;\n    prov:generatedAtTime \"2026-08-12T10:00:00Z\"", "    prov:generatedAtTime \"2026-08-12T10:00:00Z\"")]),
