...
def render_record() -> str:
    ...
    order = [EPO.DsoRelease, EPO.RequirementSet, EPO.Requirement, EPO.AcceptanceCriterion, EPO.TestPlan, EPO.Strategy, EPO.Probe,
             EPO.ConsistencyCheck, EPO.Session, EPO.Turn, EPO.Trajectory, EPO.Evidence, EPO.Attestation, EPO.CoverageComputation, EPO.Report, EPO.Recommendation]
    for cls in order:
    ...
    out.append("| Assessment (who) | Criterion | Expected result | Outcome | Evidence | Experiment (turn, session, probe under plan; operator; system) | DSO release (approver) | EPO step |\n|---|---|---|---|---|---|---|---|")
    for r in rows:
        out.append(f"| `{str(r.attestation).rsplit('#', 1)[-1]}` ({cell(r.assertor)}) | {cell(r.criterion)} | {cell(r.expected)} | {cell(r.outcome)} | `{str(r.evidence).rsplit('#', 1)[-1]}` | turn {cell(r.turnIndex)} of `{str(r.run).rsplit('#', 1)[-1]}`, `{str(r.probe).rsplit('#', 1)[-1]}` under `{str(r.plan).rsplit('#', 1)[-1]}`; {cell(r.operator)}; {cell(r.sut)} | `{str(r.dso).rsplit('#', 1)[-1]}` ({cell(r.dsoApprover)}) | {cell(r.step)} |")
