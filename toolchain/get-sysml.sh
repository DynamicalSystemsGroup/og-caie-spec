...
# verify it on every invocation: the release tarball must hash to the
# committed digest for this platform (SHA256SUMS.pinned) and the installed
# binary must hash to the committed per-platform digest (sysml-binaries.sha256).
set -euo pipefail
cd "$(dirname "$0")"
mkdir -p bin
VER="v0.4.3"
...
TMP=$(mktemp -t osml.XXXXXX)
curl -fsSL -o "$TMP" "${BASE}/${ART}"
TAR_WANT=$(grep " ${ART}\$" SHA256SUMS.pinned | awk '{print $1}')
TAR_GOT=$(sha "$TMP")
if [ -z "$TAR_WANT" ] || [ "$TAR_WANT" != "$TAR_GOT" ]; then
  echo "sysml tarball checksum mismatch (want=$TAR_WANT got=$TAR_GOT)" >&2; rm -f "$TMP"; exit 1
fi
tar xzf "$TMP" -C bin/ sysml
rm -f "$TMP"
