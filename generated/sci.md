...
| SCI-11 | EndStateGuaranteed | machine | The wiring guarantees the end state: every seam joins a supplier port to its conjugate, every port is wired once, every item kind reaches the recorder, and the seven steps produce every item kind in fixed order; a conformant record then yields traceability in both directions and coverage. |
