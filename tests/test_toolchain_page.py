...
RECIPE = ["uv sync", "bash toolchain/get-sysml.sh", "bash checks/run-checks.sh", "uv run -q ogc doctor", "uv run myst build --html", "bash scripts/copy_explorer.sh", "uv run python -m http.server -d _build/html 8000"]
