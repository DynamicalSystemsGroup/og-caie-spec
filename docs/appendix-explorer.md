# Appendix B: the knowledge graph explorer
...
evaluation), and it is regenerated by the gate and diffed byte for byte.
Every node of the record is flagged synthetic in the data, so the page can
tell synthetic content from the rest.
