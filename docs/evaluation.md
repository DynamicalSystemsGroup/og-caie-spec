...
(7.2) is the outcome inside an attestation. Its *review* (7.1) uses the
word this specification retired, which is why we say
{term}`appropriateness`. And its note on 7.3 has no term for second-party
attestation: a testing organization attesting on a sponsor's behalf
performs a {term}`second-party <second-party conformity assessment activity>`
activity even when independent of the vendor, the measles case; an audit
by a body with no user interest would be the
{term}`third-party conformity assessment activity`. The fifth step as the
command line prints it, each quote with its tag,
