...
item is a {term}`second party <second-party conformity assessment activity>`,
and a testing organization working on its behalf performs a second-party
activity even when independent of the provider. Both facts are declared in
the record, not assumed: the sponsor's signatory declares its user
interest and the testing organization's authorized representative declares
its independence of the provider, each dated. Affected stakeholders are
