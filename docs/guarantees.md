...
report summarises what was covered. The record here is one record in that sense, and the two queries
that read it, coverage and traceback, are those two purposes made
executable. The word {term}`conformance` is reclaimed on purpose for the
machine check of the record against the shapes, keeping {term}`conformity`
for the fulfilment of a requirement by the test item.
:::
...
assert that the first run conforms and is complete, that every mutation is
caught by at least one check, that two executions produce identical graphs,
and that a model with a step's output removed makes the executor refuse to
run. Two findings are stated rather than hidden: skipping the plan approval
or the access grant leaves a record the shapes accept, because the S-shapes
judge the items that exist and no shape yet requires those items to exist;
